document.addEventListener("DOMContentLoaded", () => {
                ...
                cartItem.innerHTML = `
                    ...
                    <div class="cart-item-info">
                        ...
                        <div class="quantity-control">
                            <button class="decrease-btn" data-id="${item.id}" data-ram="${item.ram}" data-color="${item.color}">-</button>
                            <p> ${item.quantity}</p>
                            <button class="increase-btn" data-id="${item.id}" data-ram="${item.ram}" data-color="${item.color}">+</button>
                        </div>
                        <button class="remove-btn" data-id="${item.id}" data-ram="${item.ram}" data-color="${item.color}">Remove</button>
                    </div>
                `;
                cartItemsContainer.appendChild(cartItem);
            });

            subtotalElement.innerText = `Subtotal: Rs. ${subtotal.toLocaleString()}`;
        }
    }
    ...
    cartItemsContainer.addEventListener('click', (e) => {
        if (e.target.classList.contains('increase-btn')) {
            ...
            const ram = e.target.getAttribute('data-ram');
            const color = e.target.getAttribute('data-color');

            const item = cart.find(item => item.id == id && item.ram == ram && item.color == color);
            if (item) {
                item.quantity++;
        ...
        if (e.target.classList.contains('remove-btn')) {
            ...
            cart = cart.filter(item => !(item.id == id && item.ram == ram && item.color == color));
            localStorage.setItem('cart', JSON.stringify(cart));
            updateCartDisplay();
        }
    });
