...
function showProductDetails(product) {
    ...
    productDetails.innerHTML = `
        <div class="product">
            ...
            <h2>${product.name}</h2>
            <p class="price" id="dynamic-price">Price: Rs. ${product.price}</p>
            
            <!-- RAM options -->
            <label for="ram-options">Choose RAM:</label>
            <select id="ram-options">
            ...
            <!-- Color options -->
            <label for="color-options">Choose Color:</label>
            ...
            <!-- Subtotal -->
            <p id="subtotal">Subtotal: Rs. ${product.price}</p>
    ...
    function updateSubtotal() {
        ...
        subtotalDisplay.innerText = `Subtotal: Rs. ${subtotal.toLocaleString()}`;
    }

    // Update price based on RAM option
    ramOptions.addEventListener('change', (e) => {
        const selectedRam = e.target.value;
        if (selectedRam === '128') {
            currentPrice = basePrice;
        } else if (selectedRam === '256') {
            currentPrice = basePrice * 1.2;
        } else if (selectedRam === '512') {
            currentPrice = basePrice * 2;
        }
        priceDisplay.innerText = `Price: Rs. ${Math.round(currentPrice).toLocaleString()}`;
        updateSubtotal();
    });
...
function addToCart(product) {
    const ramOption = document.getElementById('ram-options').value;
    const colorOption = document.getElementById('color-options').value;
    ...
    const existingItem = cart.find(item => item.id === product.id && item.ram === ramOption && item.color === colorOption);
